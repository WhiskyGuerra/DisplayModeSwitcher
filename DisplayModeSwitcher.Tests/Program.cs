...
var tests = new (string Name, Action Run)[]
{
    ...
    ("Updateprüfung akzeptiert nur das exakte Paket mit Prüfsumme", UpdateCheckerTests.AcceptsOnlyTheExactVerifiedPackagePair),
    ("Gültiges Update-Paket wird erst nach Hashprüfung bereitgestellt", UpdatePackageStagerTests.ValidPackageIsStagedAfterHashVerification),
    ("Falsche Update-Prüfsumme entfernt unvollständiges Staging", UpdatePackageStagerTests.HashMismatchRemovesIncompleteStaging),
    ("Unsichere Update-Metadaten werden vor dem Download abgelehnt", UpdatePackageStagerTests.RejectsUnsafeMetadataBeforeDownload),
    ("Abweichende Assetgröße verwirft den Update-Download", UpdatePackageStagerTests.RejectsDownloadedSizeThatDiffersFromMetadata),
    ("Nur gültige SHA-256-Werte werden akzeptiert", UpdatePackageStagerTests.ParsesOnlyValidSha256Values),
    ("Gültige Update-ZIPs werden isoliert für den Austausch vorbereitet", UpdateInstallerTests.ValidArchiveIsPreparedWithoutTouchingInstallation),
    ("Unsichere Pfade und Profildateien werden aus Update-ZIPs abgelehnt", UpdateInstallerTests.UnsafeOrProfileArchivesAreRejectedAndCleaned),
    ("Unvollständige Update-ZIPs werden vor der Installation abgelehnt", UpdateInstallerTests.MissingApplicationFilesAreRejected),
    ...
    ("Profiloberfläche unterscheidet neu, geändert, gespeichert und blockiert", ProfileManagerPresentationTests.EditorStatesAreExplicit),
    ("Profiloberfläche benennt Ziel- und Profilaktionen eindeutig", ProfileManagerPresentationTests.ActionNamesDistinguishTheirScope),
    ("Profiloberfläche kennzeichnet Lade-, Bereit- und Fehlerstatus", ProfileManagerPresentationTests.StatusTextsExposeTheirMeaning),
    ("Bestehende Monitorziele werden nur bei eindeutiger Identität ausgewählt", ProfileManagerPresentationTests.ExistingTargetSelectionIsExactAndUnambiguous),
